...
namespace Core.Abstracts.IRepositories
{
    public interface IRepositories<T> where T : BaseEntity
    {
        ...
        Task<(IEnumerable<T> Items, int TotalCount)> GetPagedAsync(int pageNumber, int pageSize, Expression<Func<T, bool>>? filter = null, Expression<Func<T, object>>? orderBy = null, params string[] includes);
